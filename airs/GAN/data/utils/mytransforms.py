...
class Resize(object):
    ...
    def __call__(self, data):
        ...
        return {
            'image': F.resize(image, self.size, interpolation=_RESAMPLE_BILINEAR),
            'label': F.resize(label, self.size, interpolation=_RESAMPLE_NEAREST),
        }
...
class RandomZoom(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < 0.5:
            ...
            zoom_label = clipped_zoom(label, zoom, order=0)

            zoom_image = Image.fromarray(zoom_image.astype('uint8'), 'RGB')
            # Label may be single-channel (L) for single-class datasets or
            # 3-channel (RGB-packed [region, ps, fh]) for PSFH multi-class.
            label_arr = zoom_label.astype('uint8')
            zoom_label = Image.fromarray(label_arr, 'RGB' if label_arr.ndim == 3 else 'L')
...
class Translation(object):
    ...
    def __call__(self, data):
        ...
        if random.random() < 0.5:
            ...
            translate_label_arr = translate_label.astype('uint8')
            translate_label = Image.fromarray(
                translate_label_arr,
                'RGB' if translate_label_arr.ndim == 3 else 'L',
            )
