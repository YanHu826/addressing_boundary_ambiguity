...
_TRAIN_SPLITS = {
    'label': {
        1: ('UDIAT', '18', 'labeled.txt'),
        2: ('UDIAT', '36', 'labeled.txt'),
        3: ('UDIAT', '73', 'labeled.txt'),
    },
    'unlabel': {
        1: ('UDIAT', '18', 'unlabeled.txt'),
        2: ('UDIAT', '36', 'unlabeled.txt'),
        3: ('UDIAT', '73', 'unlabeled.txt'),
    },
}
_DATA_SUBDIR = os.path.join('DATA', 'UDIAT')
    ...
    def __init__(self, root, expID, mode='train', ratio=10, sign='label', transform=None):
        ...
        if mode == 'train':
            ...
            if split_parts is None:
                raise ValueError(f'Unsupported expID {expID} for UDIAT.')
            ...
            imgfile = get_split_file('UDIAT', 'val.txt')
            with open(imgfile, 'r') as f:
                imglist = f.read().splitlines()
                self.imglist = [resolve_split_entry(root, img, fallback_subdir=_DATA_SUBDIR) for img in imglist]
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train' and self.sign == 'unlabel':
            ...
            if 'Benign' in img_path:
                ...
                raise ValueError(f"[ERROR] 无法识别路径中的类型: {img_path}")

            if not os.path.exists(gt_path):
                raise FileNotFoundError(f"[ERROR] mask 文件不存在: {gt_path}")
